...
const issueTypes = [
  'hardware_issue', 'software_issue', 'network_issue', 'database_issue',
  'project_related', 'user_access', 'configuration', 'other'
];

const technologyArea = [
  'resilient_connectivity', 'transformed_infrastructure', 
  'zero_trust_security', 'business_aligned_appications'
];

const teams = [
  'TSE - resilient_connectivity', 'TSE - transformed_infrastructure',
  'TSE - zero_trust_security', 'TSE - business_aligned_applications',
  'TSME - resilient_connectivity', 'TSME - transformed_infrastructure',
  'TSME - zero_trust_security', 'TSME - business_aligned_applications',
  'Service Desk'
];
...
const AdminTickets = () => {
  ...
  // Search term — what the user types in the search bar
  const [searchTerm, setSearchTerm] = useState('');

  const [form, setForm] = useState({
    title: '', description: '', type: 'incident', priority: 'minor',
  issueType: 'other', supportMode: 'remote', technologyArea: '', team: ''
  });
  ...
  const handleCreateTicket = async (e) => {
    e.preventDefault();
    try {
      // Priority only matters for incidents — don't send it otherwise
      const payload = { ...form };
  ...
  // Filter tickets based on search term BEFORE rendering
  // .toLowerCase() on both sides makes the search case-insensitive
  // We search across title, client name, and ticket type
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-lg font-medium text-nextext">
          All tickets <span className="text-gray-400 font-normal">({filteredTickets.length})</span>
        </h1>
        <button
      ...
      {/* Search bar for THIS page — separate from the top bar search, more on that below */}
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        ...
        <div className="bg-white border border-gray-200 rounded-lg px-3.5 py-3">
          ...
          <p className="text-lg font-medium text-nextext">{resolvedCount}</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filteredTickets.map((ticket) => (
              <tr key={ticket._id} className="border-b border-gray-100 last:border-0 cursor-pointer hover:bg-gray-50" onClick={() => navigate(`/admin/tickets/${ticket._id}`)}>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="New ticket">
        <form onSubmit={handleCreateTicket} className="max-h-[70vh] overflow-y-auto pr-1">

          <label className="text-xs text-gray-500 block mb-1">Title *</label>
          <input
            name="title" value={form.title} onChange={handleChange}
            placeholder="Summary of request" required
          ...
          <textarea
            ...
            placeholder="Detailed description of request" required rows={3}
            className="w-full mb-3 px-3 py-2 border border-gray-300 rounded-md text-sm"
          />
          ...
          {/* Priority only shows for incidents — matches backend rule exactly */}
          {form.type === 'incident' && (
            <>
          ...
          <label className="text-xs text-gray-500 block mb-1">Issue type</label>
          <select
          ...
          <label className="text-xs text-gray-500 block mb-1">Mode of support</label>
          <select
          ...
          <label className="text-xs text-gray-500 block mb-1">Technology area</label>
          <select
            name="technologyArea" value={form.technologyArea} onChange={handleChange}
            className="w-full mb-3 px-3 py-2 border border-gray-300 rounded-md text-sm capitalize"
          >
            <option value="">Select technology area</option>
            {technologyArea.map((ta) => (
              <option key={ta} value={ta}>{ta.replace(/_/g, ' ')}</option>
            ))}
          </select>
          ...
          <button
            type="submit"
            className="w-full bg-nexdark text-white py-2.5 rounded-md text-sm font-medium sticky bottom-0"
          >
            Create ticket
          </button>
        </form>
